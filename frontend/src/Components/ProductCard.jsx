...
const ProductCard = (props) => {
  ...
  const getHoverBorderClasses = (hoverPosition) => {
    ...
    if (hoverPosition) {
      ...
      if (isCenter) {
        classes.push("border-4 border-green-500");
      } else {
        if (x - left < 80) classes.push("border-l-4  border-green-500");
        if (right - x < 80) classes.push("border-r-4  border-green-500");
        if (y - top < 80) classes.push("border-t-4  border-green-500");
        if (bottom - y < 80) classes.push("border-b-4  border-green-500");
      }
    }
    return classes.join("");
  };
      ...
      >
        ...
        <div className="flex justify-between items-center">
          {props.discount ? (
            ...
            <span className="bg-slate-300 py-2 w-30 text-lg font-bold text-center px-4 rounded-lg text-emerald-500 hover:text-emerald-500">
              بدون تخفیف
            </span>
          )}
          <div className="">
